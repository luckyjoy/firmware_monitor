...
import os # <-- Added os import for directory handling
...
# Simulates a security alert causing the system to reject a BOOST request.
...
MOCK_LOG_DATA_EXTREME_LATENCY = [
    ...
    *[{"event": f"Task Executed {i}", "timestamp_ms": 1000 + i, "metric": {"latency_us": 100}} for i in range(20)],
...
class FirmwarePerformanceAnalyzer:
    ...
    def calculate_boot_time(self):
        ...
        for entry in self.log_data:
            ...
            try:
                ...
                if not isinstance(timestamp, (int, float)): continue
    ...
    def analyze_security_events(self):
        ...
        for entry in self.log_data:
            ...
            # Count explicit security breaches (e.g., buffer overflow flag)
    ...
    def analyze_boost_cycle_time(self):
        ...
        for entry in self.log_data:
            try:
                ...
                if current_mode == "BOOST" and boost_start_time is None:
                    # Check the previous state to ensure it was not BOOST already (simple state machine)
        ...
        if boost_start_time is not None and self.log_data:
            ...
            # Ensure the last timestamp is later than the boost start time
    ...
    def analyze_metrics(self):
        ...
        if latency_readings:
            ...
            # Handle N=0 case, though get_metrics_by_type should prevent it.
            if N == 0:
                pass
            else:
                # P95 index calculation: index of the 95th percentile value
    ...
    def run_analysis(self, scenario_name="Default"):
        ...
        # This is kept outside the standard metric groups for separate display
        boot_time = self.calculate_boot_time()
        print(f"BOOT_TIME_METRIC|Boot Time|{boot_time} seconds")
        print("-" * 60)
        ...
        if security_results:
            ...
            for key, value in security_results.items():
                print(f"SECURITY_METRIC|{key}|{value}")
            print("-" * 60)
        ...
        if boost_duration is not None:
            ...
            print(f"BOOST_METRIC|Total Boost Mode Duration|{boost_duration:.2f} seconds")
            print("-" * 60)
        ...
        for metric_name, data in metric_results.items():
            ...
            for key, value in data.items():
                # Format to two decimal places if a number
                formatted_value = f"{value:.2f}" if isinstance(value, (int, float)) else str(value)
                # Use a specific prefix for easy HTML parsing
                print(f"DETAIL_METRIC|{key} ({metric_name.split('(')[0].strip()})|{formatted_value}")
        print("\n" + "-" * 60)
...
def _text_to_html(text_report):
    ...
    for line in text_report.splitlines():
        ...
        if line.startswith("Firmware Performance Analysis Report:"):
            ...
            if in_metric_group: scenario_html_sections.append('</div>') # Close hanging metric group
            if in_scenario_card: scenario_html_sections.append('</div>') # Close previous scenario card
        ...
        # Start of summary
            ...
            if in_metric_group: scenario_html_sections.append('</div>')
            if in_scenario_card: scenario_html_sections.append('</div>')
            in_metric_group = False
            in_scenario_card = False
        ...
        # --- Content Generation (Inside Scenario Card) ---
             ...
             if line.startswith("--- "):
                 # 1. Handle sub-headers (e.g., "--- Security Analysis ---" or "--- CPU Usage (%) ---")
                 
                 if in_metric_group: 
                     scenario_html_sections.append('</div>') # Close previous metric group before starting a new one
                 ...
                 # 2. Handle simple Boot Metric (outside a group)
                 if in_metric_group:
                 ...
                 try:
                     ...
                     # Use a different class for non-group items to distinguish the simple boot time
             ...
             elif in_metric_group and (line.startswith("SECURITY_METRIC|") or line.startswith("BOOST_METRIC|") or line.startswith("DETAIL_METRIC|")):
                 # 3. Handle Group Metrics
                 try:
                     _, key, value = line.split("|", 2) 
        ...
        # --- Content Generation (Inside Summary Card) ---
            ...
            if line.startswith("| Total Tests Executed:") or \
                line.startswith("| Tests Passed:") or \
                line.startswith("| Tests Failed:") or \
                line.startswith("| Total Execution Time:"):
                
                key, value = line.strip("| ").split(":", 1)
                value_class = "summary-value time" if "Execution Time" in key else "summary-value"
                summary_html_sections.append(f'<div class="summary-item"><span class="summary-key">{key}:</span><span class="{value_class}">{value.strip()}</span></div>')
            
            # Individual test status line (This is the list that needs the grid)
            elif line.startswith("| ") and ":" in line:
                
                # Start the test results grid wrapper
                ...
                if len(parts) == 2:
                    ...
                    summary_html_sections.append(f'<div class="test-result {status_class}"><span class="test-name">{name.strip()}</span><span class="test-status">{status.strip()}</span></div>')
...
<head>
    ...
    <style>
        ...
        .report-grid {{
            /* This is the container for all individual scenario report blocks */
            display: grid;
        ...
        .metric-group {{
            /* This targets groups like "CPU Usage (%)" and "Task Latency (us)" */
            display: grid;
        ...
        .metric-item {{
            ...
            /* Remove margin-top since the grid gap handles spacing now */
        }}
...
def run_all_tests():
    ...
    # Redirect all print statements to the in-memory buffer
    sys.stdout = redirected_output
    
    # --- Add Author Information to the Start of the TXT Report ---
    print("\n" + "=" * 60)
    ...
    # 1. Define all tests (omitted for brevity, assume content is the same)
    metric_tests = [
    ...
    for name, data in metric_tests:
        try:
            ...
            print(f"CRITICAL_FAILURE|{name}|{e}") 
    ...
    try:
        ...
        print(f"| FAIL: Expected ValueError but initialization succeeded.")
